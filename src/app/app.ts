...
class ScrollingObject {
  ...
  public Update(delta: number) {
    let baseScorlledSpeed = this.solid
      ? GameApp.ScrollSpeed
      : GameApp.ScrollSpeed - 1;
    let scrollSpeed = baseScorlledSpeed + Math.min(GameApp.Score / 15.0, 1);
...
export class GameApp {
  ...
  constructor(parent: HTMLElement, width: number, height: number) {
    ...
    this.app.ticker.add((delta) => {
      ...
      // if lose show gameover, else display score
      if (!GameApp.GameOver) {
  ...
  static GetScoreNextObstacle(): number {
    let minimunDistance = 25;
    let difficulty = Math.min(this.Score / 100, 5);
    return Math.random() * 10 - difficulty * 4 + minimunDistance;
  }
